...
options {
    tokenVocab = PlSqlLexer;            // Ligar com o Lexer
    superClass = PlSqlParserBase;       // Usar a classe base correta
}
...
sql_script
    : sql_plus_command_no_semicolon? (
        (sql_plus_command | unit_statement) (SEMICOLON SLASH? (sql_plus_command | unit_statement))* SEMICOLON? SLASH?
    ) EOF
    ;
...
alter_diskgroup
    : ALTER DISKGROUP (
        ...
        | (id_expression (COMMA id_expression)* | ALL) (
            undrop_disk_clause
...
disk_online_clause
    : ONLINE (
        (
            quorum_regular? DISK id_expression (COMMA id_expression)*
...
qualified_template_clause
    : ATTRIBUTES LPAREN redundancy_clause? striping_clause? RPAREN //inconsistent documentation
    ;
...
diskgroup_attributes
    : SET ATTRIBUTE CHAR_STRING EQUAL CHAR_STRING
    ;

modify_diskgroup_file
    : MODIFY FILE CHAR_STRING ATTRIBUTE LPAREN disk_region_clause RPAREN (
        COMMA CHAR_STRING ATTRIBUTE LPAREN disk_region_clause RPAREN
    )*
    ;
...
file_permissions_clause
    : SET PERMISSION (OWNER | GROUP | OTHER) EQUAL (NONE | READ (ONLY | WRITE)) (
        COMMA (OWNER | GROUP | OTHER) EQUAL (NONE | READ (ONLY | WRITE))
    )* FOR FILE CHAR_STRING (COMMA CHAR_STRING)*
    ;

file_owner_clause
    : SET OWNERSHIP (OWNER | GROUP) EQUAL CHAR_STRING (COMMA (OWNER | GROUP) EQUAL CHAR_STRING)* FOR FILE CHAR_STRING (
        COMMA CHAR_STRING
    )*
    ;
...
quotagroup_clauses
    : ADD QUOTAGROUP id_expression (SET property_name EQUAL property_value)?
    | MODIFY QUOTAGROUP id_expression SET property_name EQUAL property_value
...
add_filegroup_clause
    ...
    )? (SET CHAR_STRING EQUAL CHAR_STRING)?
    ;

modify_filegroup_clause
    : MODIFY FILEGROUP id_expression SET CHAR_STRING EQUAL CHAR_STRING
    ;
...
alter_hierarchy
    : ALTER HIERARCHY (schema_name DOT)? hn = id_expression (
...
alter_java
    : ALTER JAVA (SOURCE | CLASS) (schema_name DOT)? o = id_expression (
        RESOLVER LPAREN (LPAREN match_string COMMA? (schema_name | MINUS) RPAREN)+ RPAREN
...
create_function_body
    ...
    )? RETURN type_spec (SHARING EQUAL (METADATA | NONE))? (
...
partition_by_clause
    : LPAREN PARTITION expression BY (ANY | (HASH | RANGE | LIST) paren_column_list) streaming_clause? RPAREN
    ;

result_cache_clause
    : RESULT_CACHE relies_on_part? (LPAREN MODE (DEFAULT | FORCE ) RPAREN)?
    ;

accessible_by_clause
    : ACCESSIBLE BY LPAREN accessor (COMMA accessor)* RPAREN
    ;
...
relies_on_part
    : RELIES_ON LPAREN tableview_name (COMMA tableview_name)* RPAREN
    ;
...
alter_lockdown_profile
    : ALTER LOCKDOWN PROFILE id_expression (
        ...
        | lockdown_options
        | lockdown_statements
    ) (USERS EQUAL (ALL | COMMON | LOCAL))?
    ;

lockdown_feature
    : disable_enable FEATURE (EQUAL LPAREN string_list RPAREN | ALL (EXCEPT EQUAL LPAREN string_list RPAREN)?)
    ;

lockdown_options
    : disable_enable OPTION (EQUAL LPAREN string_list RPAREN | ALL (EXCEPT EQUAL LPAREN string_list RPAREN)?)
    ;

lockdown_statements
    : disable_enable STATEMENT (
        EQUAL LPAREN string_list RPAREN
        | EQUAL LPAREN CHAR_STRING RPAREN statement_clauses
        | ALL (EXCEPT EQUAL LPAREN string_list RPAREN)?
    )
    ;
...
statement_clauses
    : CLAUSE (
        EQUAL LPAREN string_list RPAREN
        | EQUAL LPAREN CHAR_STRING RPAREN clause_options
        | ALL (EXCEPT EQUAL LPAREN string_list RPAREN)?
    )
    ;
...
clause_options
    : OPTION (
        EQUAL LPAREN string_list RPAREN
        | EQUAL LPAREN CHAR_STRING RPAREN option_values+
        | ALL (EXCEPT EQUAL LPAREN string_list RPAREN)?
    )
    ;

option_values
    : VALUE EQUAL LPAREN string_list RPAREN
    | (MINVALUE | MAXVALUE) EQUAL CHAR_STRING
    ;
...
drop_package
    : DROP PACKAGE BODY? (schema_object_name DOT)? package_name
    ;
...
create_package
    : CREATE (OR REPLACE)? (EDITIONABLE | NONEDITIONABLE)? PACKAGE (schema_object_name DOT)? package_name invoker_rights_clause? (
        IS
        | AS
...
create_package_body
    : CREATE (OR REPLACE)? (EDITIONABLE | NONEDITIONABLE)? PACKAGE BODY (schema_object_name DOT)? package_name (
        IS
        | AS
...
procedure_spec
    : PROCEDURE identifier (LPAREN parameter ( COMMA parameter)* RPAREN)? (
        accessible_by_clause
        | PARALLEL_ENABLE
    )* SEMICOLON
    ;
...
function_spec
    : FUNCTION identifier (LPAREN parameter ( COMMA parameter)* RPAREN)? RETURN type_spec (
        DETERMINISTIC
        | PIPELINED
        | parallel_enable_clause
        | RESULT_CACHE
...
create_procedure_body
    : CREATE (OR REPLACE)? PROCEDURE procedure_name (LPAREN parameter (COMMA parameter)* RPAREN)? invoker_rights_clause? PARALLEL_ENABLE? (
        IS
        | AS
...
non_dml_trigger
    : (BEFORE | AFTER) non_dml_event (OR non_dml_event)* ON (DATABASE | (schema_name DOT)? SCHEMA)
    ;
...
replace_type_clause
    : REPLACE invoker_rights_clause? AS OBJECT LPAREN object_member_spec (COMMA object_member_spec)* RPAREN
    ;
...
alter_attribute_definition
    : (ADD | MODIFY | DROP) ATTRIBUTE (
        ...
        | LPAREN attribute_definition (COMMA attribute_definition)* RPAREN
    )
    ;
...
object_type_def
    : invoker_rights_clause? (object_as_part | object_under_part) sqlj_object_type? (
        LPAREN object_member_spec (COMMA object_member_spec)* RPAREN
    )? modifier_clause*
    ;
...
proc_decl_in_type
    : PROCEDURE procedure_name LPAREN type_elements_parameter (COMMA type_elements_parameter)* RPAREN (
        IS
        | AS
...
func_decl_in_type
    : FUNCTION function_name (LPAREN type_elements_parameter (COMMA type_elements_parameter)* RPAREN)? RETURN type_spec (
        IS
        | AS
    ) (call_spec | DECLARE? seq_of_declare_specs? body SEMICOLON)
    ;
...
constructor_declaration
    : FINAL? INSTANTIABLE? CONSTRUCTOR FUNCTION type_spec (
        LPAREN (SELF IN OUT type_spec COMMA) type_elements_parameter (COMMA type_elements_parameter)* RPAREN
...
overriding_function_spec
    : FUNCTION function_name (LPAREN type_elements_parameter (COMMA type_elements_parameter)* RPAREN)? RETURN (
        type_spec
        | SELF AS RESULT
...
pragma_clause
    : PRAGMA RESTRICT_REFERENCES LPAREN pragma_elements (COMMA pragma_elements)* RPAREN
    ;
...
alter_session_set_clause
    : (parameter_name EQUAL parameter_value)+
    | EDITION EQUAL en = id_expression
    | CONTAINER EQUAL cn = id_expression (SERVICE EQUAL sn = id_expression)?
    | ROW ARCHIVAL VISIBILITY EQUAL (ACTIVE | ALL)
    | DEFAULT_COLLATION EQUAL (c = id_expression | NONE)
    ;
...
cav_using_clause
    : USING (schema_name DOT)? t = id_expression REMOTE? (AS? ta = id_expression)?
    ;

dim_by_clause
    : DIMENSION BY LPAREN dim_key (COMMA dim_key)* RPAREN
    ;

dim_key
    : dim_ref classification_clause* KEY (
        LPAREN (a = id_expression DOT)? f = column_name (COMMA (a = id_expression DOT)? f = column_name)* RPAREN
        | (a = id_expression DOT)? f = column_name
    ) REFERENCES DISTINCT? (LPAREN attribute_name (COMMA attribute_name) RPAREN | attribute_name) HIERARCHIES LPAREN hier_ref (
        COMMA hier_ref
    )* RPAREN
    ;

dim_ref
    : (schema_name DOT)? ad = id_expression (AS? da = id_expression)?
    ;

hier_ref
    : (schema_name DOT)? h = id_expression (AS? ha = id_expression)? DEFAULT?
    ;

measures_clause
    : MEASURES LPAREN av_measure (COMMA av_measure)* RPAREN
    ;
...
calc_meas_clause
    : /*m=id_expression*/ AS LPAREN expression RPAREN //FIXME inconsistent documentation
    ;
...
cache_specification
    : MEASURE GROUP (
        ALL
        | LPAREN id_expression (COMMA id_expression)* RPAREN levels_clause (COMMA levels_clause)*
    )
    ;

levels_clause
    : LEVELS LPAREN level_specification (COMMA level_specification)* RPAREN level_group_type
    ;

level_specification
    : LPAREN ((d = id_expression DOT)? h = id_expression DOT)? l = id_expression RPAREN
    ;

level_group_type
    : DYNAMIC
    | MATERIALIZED (USING (schema_name DOT)? t = id_expression)?
    ;
...
create_attribute_dimension
    : CREATE (OR REPLACE)? (NOFORCE | FORCE)? ATTRIBUTE DIMENSION (schema_name DOT)? ad = id_expression (
        SHARING EQUAL (METADATA | NONE)
...
source_clause
    : (schema_name DOT)? ftov = id_expression REMOTE? (AS? a = id_expression)?
    ;
...
join_condition_item
    : (a = id_expression DOT)? column_name EQUAL (b = id_expression DOT)? column_name
    ;

attributes_clause
    : ATTRIBUTES LPAREN ad_attributes_clause (COMMA ad_attributes_clause)* RPAREN
    ;

ad_attributes_clause
    : (a = id_expression DOT)? column_name (AS? an = id_expression)? classification_clause*
    ;
...
ad_level_clause
    ...
    key_clause alternate_key_clause? (MEMBER NAME expression)? (MEMBER CAPTION expression)? (
        ...
        DETERMINES LPAREN id_expression (COMMA id_expression)* RPAREN
    )?
    ;

key_clause
    : KEY (a = id_expression | LPAREN id_expression (COMMA id_expression)* RPAREN)
    ;
...
actions_clause
    : (object_action | ALL) ON (
        ...
        | (MINING MODEL)? (schema_name DOT)? id_expression
    )
    | (system_action | ALL)
    ;
...
create_diskgroup
    ...
    )+ (ATTRIBUTE an = CHAR_STRING EQUAL av = CHAR_STRING (COMMA CHAR_STRING EQUAL CHAR_STRING)*)?
    ;
...
create_hierarchy
    : CREATE (OR REPLACE)? (NO? FORCE)? HIERARCHY (schema_name DOT)? h = id_expression (
        SHARING EQUAL (METADATA | NONE)
    )? classification_clause* hier_using_clause level_hier_clause hier_attrs_clause?
    ;

hier_using_clause
    : USING (schema_name DOT)? ad = id_expression
    ;

level_hier_clause
    : LPAREN (l = id_expression (CHILD OF)?)+ RPAREN
    ;

hier_attrs_clause
    : HIERARCHICAL ATTRIBUTES LPAREN hier_attr_clause RPAREN
    ;
...
cluster_name
    : (id_expression DOT)? id_expression
    ;

table_index_clause
    : tableview_name table_alias? LPAREN index_expr (ASC | DESC)? (COMMA index_expr (ASC | DESC)?)* RPAREN index_properties?
    ;

bitmap_join_index_clause
    : tableview_name LPAREN (tableview_name | table_alias)? column_name (ASC | DESC)? (
        COMMA (tableview_name | table_alias)? column_name (ASC | DESC)?
    )* RPAREN FROM tableview_name table_alias (COMMA tableview_name table_alias)* where_clause local_partitioned_index? index_attributes?
    ;
...
domain_index_clause
    : indextype local_domain_index_clause? parallel_clause? (PARAMETERS LPAREN odci_parameters RPAREN)?
    ;

local_domain_index_clause
    : LOCAL (
        LPAREN PARTITION partition_name (PARAMETERS LPAREN odci_parameters RPAREN)? (
            COMMA PARTITION partition_name (PARAMETERS LPAREN odci_parameters RPAREN)?
        )* RPAREN
    )?
    ;

xmlindex_clause
    : (XDB DOT)? XMLINDEX local_xmlindex_clause? parallel_clause? //TODO xmlindex_parameters_clause?
    ;

local_xmlindex_clause
    : LOCAL (
        LPAREN PARTITION partition_name (
...
global_partitioned_index
    : GLOBAL PARTITION BY (
        RANGE LPAREN column_name (COMMA column_name)* RPAREN LPAREN index_partitioning_clause (
            COMMA index_partitioning_clause
        )* RPAREN
        | HASH LPAREN column_name (COMMA column_name)* RPAREN (
...
index_partitioning_clause
    : PARTITION partition_name? VALUES LESS THAN LPAREN index_partitioning_values_list RPAREN segment_attributes_clause?
    ;
...
on_range_partitioned_table
    : LPAREN partitioned_table (COMMA partitioned_table)* RPAREN
    ;
...
on_hash_partitioned_table
    : STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN
    | LPAREN on_hash_partitioned_clause (COMMA on_hash_partitioned_clause)* RPAREN
    ;
...
on_comp_partitioned_table
    : (STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN)? LPAREN on_comp_partitioned_clause (
...
index_subpartition_clause
    : STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN
    | LPAREN index_subpartition_subclause (COMMA index_subpartition_subclause)* RPAREN
    ;
...
split_index_partition
    : SPLIT PARTITION partition_name_old AT LPAREN literal (COMMA literal)* RPAREN (
        INTO LPAREN index_partition_description COMMA index_partition_description RPAREN
    )? parallel_clause?
    ;
...
alter_inmemory_join_group
    : ALTER INMEMORY JOIN GROUP (schema_name DOT)? jg = id_expression (ADD | REMOVE) LPAREN (
        schema_name DOT
    )? t = id_expression LPAREN c = id_expression RPAREN RPAREN
    ;
...
export_keys
    : EXPORT ENCRYPTION? KEYS WITH SECRET secret TO filename force_keystore? identified_by_store (
        WITH IDENTIFIER IN (CHAR_STRING (COMMA CHAR_STRING)* | LPAREN subquery RPAREN)
    )?
    ;
...
partition_extention_clause
    : PARTITION (
        LPAREN partition_name RPAREN
        | FOR LPAREN partition_key_value (COMMA partition_key_value)* RPAREN
    )
    | SUBPARTITION (
        LPAREN subpartition_name RPAREN
        | FOR LPAREN subpartition_key_value (COMMA subpartition_key_value)* RPAREN
    )
    ;
...
column_association
    : COLUMNS tableview_name DOT column_name (COMMA tableview_name DOT column_name)* using_statistics_type
    ;
...
default_cost_clause
    : DEFAULT COST LPAREN cpu_cost COMMA io_cost COMMA network_cost RPAREN
    ;
...
disassociate_statistics
    : DISASSOCIATE STATISTICS FROM (
        COLUMNS (schema_name DOT)? tb = id_expression DOT c = id_expression (
            COMMA (schema_name DOT)? tb = id_expression DOT c = id_expression
        )*
        | FUNCTIONS (schema_name DOT)? fn = id_expression (
            COMMA (schema_name DOT)? fn = id_expression
        )*
        | PACKAGES (schema_name DOT)? pkg = id_expression (
            COMMA (schema_name DOT)? pkg = id_expression
        )*
        | TYPES (schema_name DOT)? t = id_expression (COMMA (schema_name DOT)? t = id_expression)*
        | INDEXES (schema_name DOT)? ix = id_expression (COMMA (schema_name DOT)? ix = id_expression)*
        | INDEXTYPES (schema_name DOT)? it = id_expression (
            COMMA (schema_name DOT)? it = id_expression
        )*
    ) FORCE?
    ;
...
drop_indextype
    : DROP INDEXTYPE (schema_name DOT)? it = id_expression FORCE?
    ;

// https://docs.oracle.com/en/database/oracle/oracle-database/21/sqlrf/DROP-INMEMORY-JOIN-GROUP.html
drop_inmemory_join_group
    : DROP INMEMORY JOIN GROUP (schema_name DOT)? jg = id_expression
    ;
...
on_object_clause
    : ON (
        (schema_name DOT)? o = id_expression
        | USER id_expression (COMMA id_expression)*
        | DIRECTORY directory_name
        | EDITION edition_name
        | MINING MODEL (schema_name DOT)? mmn = id_expression
        | JAVA (SOURCE | RESOURCE) (schema_name DOT)? o2 = id_expression
        | SQL TRANSLATION PROFILE (schema_name DOT)? p = id_expression
    )
    ;
...
program_unit
    : (FUNCTION | PROCEDURE | PACKAGE) (schema_name DOT)? id_expression
    ;
...
create_directory
    : CREATE (OR REPLACE)? DIRECTORY directory_name (SHARING EQUAL (METADATA | NONE))? AS directory_path
    ;
...
create_inmemory_join_group
    : CREATE INMEMORY JOIN GROUP (schema_name DOT)? jg = id_expression LPAREN (schema_name DOT)? t = id_expression LPAREN c = id_expression RPAREN (
        COMMA (schema_name DOT)? t = id_expression LPAREN c = id_expression RPAREN
    )+ RPAREN
    ;

// https://docs.oracle.com/en/database/oracle/oracle-database/21/sqlrf/DROP-HIERARCHY.html
drop_hierarchy
    : DROP HIERARCHY (schema_name DOT)? hn = id_expression
    ;
...
drop_java
    : DROP JAVA (SOURCE | CLASS | RESOURCE) (schema_name DOT)? id_expression
    ;
...
create_java
    : CREATE (OR REPLACE)? (AND (RESOLVE | COMPILE))? NOFORCE? JAVA (
        (SOURCE | RESOURCE) NAMED (schema_name DOT)? pn = id_expression
        | CLASS (SCHEMA id_expression)?
    ) (SHARING EQUAL (METADATA | NONE))? invoker_rights_clause? (
        RESOLVER LPAREN (LPAREN CHAR_STRING COMMA? (sn = id_expression | MINUS) RPAREN)+ RPAREN
    )? (
        USING (
            BFILE LPAREN d = id_expression COMMA filename RPAREN
...
level_clause
    : LEVEL identifier IS (
        ...
        | LPAREN table_name DOT column_name (COMMA table_name DOT column_name)* RPAREN
    ) (SKIP_ WHEN NULL_)?
    ;

hierarchy_clause
    : HIERARCHY identifier LPAREN identifier (CHILD OF identifier)+ dimension_join_clause? RPAREN
    ;
...
alter_view
    : ALTER VIEW tableview_name (
        ...
        | DROP (
            ...
            | PRIMARY KEY
            | UNIQUE LPAREN column_name (COMMA column_name)* RPAREN
        )
        | COMPILE
        | READ (ONLY | WRITE)
...
create_view
    : CREATE (OR REPLACE)? (NO? FORCE)? editioning_clause? VIEW (schema_name DOT)? v = id_expression (
        IF NOT EXISTS
    )? (SHARING EQUAL (METADATA | EXTENDED? DATA | NONE))? view_options? (
...
view_alias_constraint
    : LPAREN (COMMA? (table_alias inline_constraint* | out_of_line_constraint))+ RPAREN
    ;

object_view_clause
    : OF (schema_name DOT)? tn = id_expression (
        WITH OBJECT (IDENTIFIER | ID) (DEFAULT | LPAREN REGULAR_ID (COMMA REGULAR_ID)* RPAREN)
        | UNDER (schema_name DOT)? sv = id_expression
    ) (LPAREN (COMMA? (out_of_line_constraint | REGULAR_ID inline_constraint))+ RPAREN)*
    ;
...
out_of_line_ref_constraint
    : SCOPE FOR LPAREN ref_col_or_attr = regular_id RPAREN IS tableview_name
    | REF LPAREN ref_col_or_attr = regular_id RPAREN WITH ROWID
    | (CONSTRAINT constraint_name)? FOREIGN KEY LPAREN (COMMA? ref_col_or_attr = regular_id)+ RPAREN references_clause constraint_state?
    ;
...
out_of_line_constraint
    : (
        ((CONSTRAINT | CONSTRAINTS) constraint_name)? (
            UNIQUE LPAREN column_name (COMMA column_name)* RPAREN
            | PRIMARY KEY LPAREN column_name (COMMA column_name)* RPAREN
            | foreign_key_clause
...
xml_schema_spec
    : (XMLSCHEMA xml_schema_url)? ELEMENT (element | xml_schema_url HASH element) (
...
create_tablespace_set
    : CREATE TABLESPACE SET tss = id_expression (IN SHARDSPACE ss = id_expression)? (
        USING TEMPLATE LPAREN (DATAFILE file_specification (COMMA file_specification)*)? permanent_tablespace_attrs+ RPAREN
    )?
    ;
...
redo_log_file_spec
    : (filename | LPAREN filename (COMMA filename)* RPAREN) (SIZE size_clause)? (BLOCKSIZE size_clause)? REUSE?
    ;
...
modify_mv_column_clause
    : MODIFY LPAREN column_name (ENCRYPT encryption_spec | DECRYPT)? RPAREN
    ;
...
mv_log_augmentation
    : ADD (
        (OBJECT ID | PRIMARY KEY | ROWID | SEQUENCE) (LPAREN column_name (COMMA column_name)* RPAREN)?
        | LPAREN column_name (COMMA column_name)* RPAREN
    ) new_values_clause?
    ;
...
create_materialized_view_log
    ...
    (
        WITH (COMMA? ( OBJECT ID | PRIMARY KEY | ROWID | SEQUENCE | COMMIT SCN))* (
            LPAREN ( COMMA? regular_id)+ RPAREN new_values_clause?
        )? mv_log_purge_clause?
    )*
    ;
...
create_materialized_view
    : CREATE MATERIALIZED VIEW tableview_name (OF type_name)? (
        LPAREN (scoped_table_ref_constraint | mv_column_alias) (
...
scoped_table_ref_constraint
    : SCOPE FOR LPAREN ref_column_or_attribute = identifier RPAREN IS (schema_name DOT)? scope_table_name_or_c_alias = identifier
    ;
...
create_context
    : CREATE (OR REPLACE)? CONTEXT oracle_namespace USING (schema_object_name DOT)? package_name (
...
create_cluster
    : CREATE CLUSTER cluster_name LPAREN column_name datatype SORT? (COMMA column_name datatype SORT?)* RPAREN (
        physical_attributes_clause
        | SIZE size_clause
        | TABLESPACE tablespace
        | INDEX
...
create_table
    : CREATE (
        ...
        | IMMUTABLE
    )? TABLE (schema_name DOT)? table_name (IF NOT EXISTS)? (
        SHARING EQUAL (METADATA | EXTENDED? DATA | NONE)
...
xmltype_table
    : OF XMLTYPE (LPAREN object_properties RPAREN)? (XMLTYPE xmltype_storage)? xmlschema_spec? xmltype_virtual_columns? (
        ON COMMIT (DELETE | PRESERVE) ROWS
    )? oid_clause? oid_index_clause? physical_properties? table_properties
    ;

xmltype_virtual_columns
    : VIRTUAL COLUMNS LPAREN column_name AS LPAREN expression RPAREN (COMMA column_name AS LPAREN expression RPAREN)* RPAREN
    ;
...
xmltype_storage
    : STORE AS (
        ...
        | (SECUREFILE | BASICFILE)? (CLOB | BINARY XML) (
            lob_segname (LPAREN lob_parameters RPAREN)?
            | LPAREN lob_parameters RPAREN
        )?
    )
...
object_table
    : OF (schema_name DOT)? object_type object_table_substitution? (
        LPAREN object_properties (COMMA object_properties)* RPAREN
...
oid_index_clause
    : OIDINDEX index_name? LPAREN (physical_attributes_clause | TABLESPACE tablespace)+ RPAREN
    ;
...
relational_table
    : (LPAREN relational_property (COMMA relational_property)* RPAREN)? immutable_table_clauses blockchain_table_clauses? (
...
table_properties
    : column_properties? read_only_clause? indexing_clause? table_partitioning_clauses? attribute_clustering_clause? (
        ...
        | FOR EXCHANGE WITH TABLE (schema_name DOT)? table_name
    )?
    ;
...
clustering_join
    : (schema_name DOT)? table_name clustering_join_item (COMMA clustering_join_item)*
    ;

clustering_join_item
    : JOIN (schema_name DOT)? table_name ON LPAREN equijoin_condition RPAREN
    ;
...
clustering_columns
    ...
    | LPAREN clustering_column_group (COMMA clustering_column_group)* RPAREN
    ;
...
zonemap_clause
    : WITH MATERIALIZED ZONEMAP (LPAREN zonemap_name RPAREN)?
...
range_partitions
    : PARTITION BY RANGE LPAREN column_name (COMMA column_name)* RPAREN (
        INTERVAL LPAREN expression RPAREN (STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN)?
    )? LPAREN PARTITION partition_name? range_values_clause table_partition_description (
        COMMA PARTITION partition_name? range_values_clause table_partition_description
    )* RPAREN
    ;

list_partitions
    : PARTITION BY LIST LPAREN column_name RPAREN LPAREN PARTITION partition_name? list_values_clause table_partition_description (
        COMMA PARTITION partition_name? list_values_clause table_partition_description
    )* RPAREN
    ;

hash_partitions
    : PARTITION BY HASH LPAREN column_name (COMMA column_name)* RPAREN (
        individual_hash_partitions
        | hash_partitions_by_quantity
    )
    ;
...
individual_hash_partitions
    : LPAREN PARTITION partition_name? partitioning_storage_clause? (
        COMMA PARTITION partition_name? partitioning_storage_clause?
    )* RPAREN
    ;

hash_partitions_by_quantity
    : PARTITIONS hash_partition_quantity (STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN)? (
        table_compression
        | key_compression
    )? (OVERFLOW_ STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN)?
    ;
...
composite_range_partitions
    : PARTITION BY RANGE LPAREN column_name (COMMA column_name)* RPAREN (
        INTERVAL LPAREN expression RPAREN (STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN)?
    )? (subpartition_by_range | subpartition_by_list | subpartition_by_hash) LPAREN range_partition_desc (
        COMMA range_partition_desc
    )* RPAREN
    ;

composite_list_partitions
    : PARTITION BY LIST LPAREN column_name RPAREN (
        subpartition_by_range
        | subpartition_by_list
        | subpartition_by_hash
    ) LPAREN list_partition_desc (COMMA list_partition_desc)* RPAREN
    ;

composite_hash_partitions
    : PARTITION BY HASH LPAREN (COMMA column_name)+ RPAREN (
        subpartition_by_range
        | subpartition_by_list
        | subpartition_by_hash
...
reference_partitioning
    : PARTITION BY REFERENCE LPAREN regular_id RPAREN (
        LPAREN reference_partition_desc (COMMA reference_partition_desc)* RPAREN
    )?
    ;
...
subpartition_by_range
    : SUBPARTITION BY RANGE LPAREN column_name (COMMA column_name)* RPAREN subpartition_template?
    ;

subpartition_by_list
    : SUBPARTITION BY LIST LPAREN column_name RPAREN subpartition_template?
    ;

subpartition_by_hash
    : SUBPARTITION BY HASH LPAREN column_name (COMMA column_name)* RPAREN (
        SUBPARTITIONS UNSIGNED_INTEGER (STORE IN LPAREN tablespace (COMMA tablespace)* RPAREN)?
        | subpartition_template
    )?
    ;
...
range_values_clause
    : VALUES LESS THAN LPAREN range_values_list RPAREN
    ;
...
list_values_clause
    : VALUES LPAREN (literal (COMMA literal)* | TIMESTAMP literal (COMMA TIMESTAMP literal)* | DEFAULT) RPAREN
    ;
...
lob_partitioning_storage
    : LOB LPAREN lob_item RPAREN STORE AS (BASICFILE | SECUREFILE)? (
        lob_segname (LPAREN TABLESPACE tablespace RPAREN)?
        | LPAREN TABLESPACE tablespace RPAREN
    )
    ;
...
inmemory_column_clause
    : (INMEMORY inmemory_memcompress? | NO INMEMORY) LPAREN column_list RPAREN
    ;
...
physical_properties
    ...
    | CLUSTER cluster_name LPAREN column_name (COMMA column_name)* RPAREN
    ;
...
external_table_clause
    : LPAREN (TYPE access_driver_type)? external_table_data_props RPAREN (
...
external_table_data_props
    : (DEFAULT DIRECTORY directory_name)? (
        ACCESS PARAMETERS (
            LPAREN CHAR_STRING RPAREN
            | LPAREN opaque_format_spec RPAREN
            | USING CLOB select_only_statement
        )
    )? (LOCATION LPAREN directory_name COLON CHAR_STRING (COMMA directory_name COLON CHAR_STRING)* RPAREN)?
    ;
...
supplemental_log_grp_clause
    : GROUP log_grp LPAREN regular_id (NO LOG)? (COMMA regular_id (NO LOG)?)* RPAREN ALWAYS?
    ;

supplemental_id_key_clause
    : DATA LPAREN (COMMA? ( ALL | PRIMARY KEY | UNIQUE | FOREIGN KEY))+ RPAREN COLUMNS
    ;
...
alter_synonym
    : ALTER PUBLIC? SYNONYM (schema_name DOT)? synonym_name (
        EDITIONABLE
        | NONEDITIONABLE
        | COMPILE
    )
...
drop_synonym
    : DROP PUBLIC? SYNONYM (schema_name DOT)? synonym_name FORCE?
    ;

// https://docs.oracle.com/en/database/oracle/oracle-database/21/sqlrf/CREATE-SPFILE.html
create_spfile
    : CREATE SPFILE (EQUAL spfile_name)? FROM (PFILE (EQUAL pfile_name)? (AS COPY)? | MEMORY)
    ;
...
alter_analytic_view
    : ALTER ANALYTIC VIEW (schema_name DOT)? av = id_expression (
...
alter_add_cache_clause
    : ADD CACHE MEASURE GROUP LPAREN (ALL | measure_list)? RPAREN LEVELS LPAREN levels_item (
        COMMA levels_item
    )* RPAREN
    ;

levels_item
    : ((d = id_expression DOT)? h = id_expression DOT)? l = id_expression
    ;
...
alter_drop_cache_clause
    : DROP CACHE MEASURE GROUP LPAREN (ALL | measure_list)? RPAREN LEVELS LPAREN levels_item (
        COMMA levels_item
    )* RPAREN
    ;
...
alter_attribute_dimension
    : ALTER ATTRIBUTE DIMENSION (schema_name DOT)? ad = id_expression (
...
drop_analytic_view
    : DROP ANALYTIC VIEW (schema_name DOT)? av = id_expression
    ;

// https://docs.oracle.com/en/database/oracle/oracle-database/21/sqlrf/DROP-ATTRIBUTE-DIMENSION.html
drop_attribute_dimension
    : DROP ATTRIBUTE DIMENSION (schema_name DOT)? ad = id_expression
    ;
...
logfile_descriptor
    ...
    | LPAREN filename (COMMA filename)* RPAREN
    | filename
    ;
...
default_settings_clause
    ...
    | RENAME GLOBAL_NAME TO database (DOT domain)+
...
lead_cdb_clause
    : SET LEAD_CDB EQUAL (TRUE | FALSE)
    ;

lead_cdb_uri_clause
    : SET LEAD_CDB_URI EQUAL CHAR_STRING
    ;

property_clauses
    : PROPERTY (SET | REMOVE) DEFAULT_CREDENTIAL EQUAL qcn = id_expression
    ;
...
file_name_convert
    : FILE_NAME_CONVERT EQUALS_OP (
        LPAREN filename_convert_sub_clause (COMMA filename_convert_sub_clause)* RPAREN
        | NONE
    )
    ;
...
ts_file_name_convert
    : FILE_NAME_CONVERT EQUAL LPAREN CHAR_STRING COMMA CHAR_STRING (COMMA CHAR_STRING COMMA CHAR_STRING)* RPAREN KEEP?
    ;
...
role_identified_clause
    ...
    | IDENTIFIED (
        BY identifier
        | USING identifier (DOT id_expression)?
        | EXTERNALLY
...
split_table_partition
    : SPLIT partition_extended_names (
        AT LPAREN literal (COMMA literal)* RPAREN INTO LPAREN range_partition_desc (COMMA range_partition_desc)* RPAREN
        | INTO LPAREN (
...
alter_interval_partition
    : SET INTERVAL LPAREN (constant | expression)? RPAREN
    ;

partition_extended_names
    : (PARTITION | PARTITIONS) (
        partition_name (COMMA partition_name)*
        | LPAREN partition_name (COMMA partition_name)* RPAREN
        | FOR LPAREN? partition_key_value (COMMA partition_key_value)* RPAREN?
    )
    ;
...
alter_table_properties_1
    : (
        ...
        | deallocate_unused_clause
        | (CACHE | NOCACHE)
        | RESULT_CACHE LPAREN MODE (DEFAULT | FORCE) RPAREN
        | upgrade_table_clause
        | records_per_block_clause
        | parallel_clause
        | row_movement_clause
...
add_overflow_clause
    : ADD OVERFLOW_ segment_attributes_clause? (
        LPAREN PARTITION segment_attributes_clause? (COMMA PARTITION segment_attributes_clause?)* RPAREN
    )?
    ;
...
update_all_indexes_clause
    : UPDATE INDEXES (LPAREN update_all_indexes_index_clause RPAREN)?
    ;

update_all_indexes_index_clause
    : index_name LPAREN (update_index_partition | update_index_subpartition) RPAREN (
...
using_index_clause
    : USING INDEX (index_name | LPAREN create_index RPAREN | index_properties)
    ;
...
drop_column_clause
    : SET UNUSED (COLUMN column_name | (LPAREN column_name (COMMA column_name)* RPAREN)) (
        CASCADE CONSTRAINTS
        | INVALIDATE
    )*
    | DROP (COLUMN column_name | LPAREN column_name (COMMA column_name)* RPAREN) (
        CASCADE CONSTRAINTS
        | INVALIDATE
...
modify_column_clauses
    : MODIFY (
        LPAREN modify_col_properties (COMMA modify_col_properties)* RPAREN
        | LPAREN modify_col_visibility (COMMA modify_col_visibility)* RPAREN
...
add_column_clause
    : ADD (
        LPAREN (column_definition | virtual_column_definition) (
...
alter_varray_col_properties
    : MODIFY VARRAY varray_item LPAREN modify_lob_parameters RPAREN
    ;
...
varray_storage_clause
    : STORE AS (SECUREFILE | BASICFILE)? LOB (
        lob_segname? LPAREN lob_storage_parameters RPAREN
        | lob_segname
    )
    ;
...
lob_storage_clause
    : LOB (
        LPAREN lob_item (COMMA lob_item)* RPAREN STORE AS (
            (SECUREFILE | BASICFILE)
            | LPAREN lob_storage_parameters* RPAREN
        )+
        | LPAREN lob_item RPAREN STORE AS (
            (SECUREFILE | BASICFILE)
            | lob_segname
            | LPAREN lob_storage_parameters* RPAREN
        )+
...
modify_lob_storage_clause
    : MODIFY LOB LPAREN lob_item RPAREN LPAREN modify_lob_parameters RPAREN
    ;
...
varray_item
    : (id_expression DOT)? (id_expression DOT)? id_expression
    ;
...
column_properties
    : (
        ...
        | (varray_col_properties | lob_storage_clause) (
            LPAREN lob_partition_storage (COMMA lob_partition_storage)* RPAREN
        )? //TODO LPAREN ( COMMA? lob_partition_storage)+ RPAREN
        | xmltype_column_properties
    )+
    ;
...
period_definition
    : {this.isVersion12()}? PERIOD FOR column_name (LPAREN start_time_column COMMA end_time_column RPAREN)?
    ;
...
identity_options_parentheses
    : identity_options+
    | LPAREN identity_options+ RPAREN
    ;
...
autogenerated_sequence_definition
    : GENERATED (ALWAYS | BY DEFAULT (ON NULL_)?)? AS IDENTITY (
        LPAREN (sequence_start_clause | sequence_spec)* RPAREN
    )?
    ;
...
nested_table_col_properties
    : NESTED TABLE (nested_item | COLUMN_VALUE) substitutable_column_clause? (LOCAL | GLOBAL)? STORE AS tableview_name (
        LPAREN (LPAREN object_properties RPAREN | physical_properties | column_properties)+ RPAREN
...
substitutable_column_clause
    : ELEMENT? IS OF TYPE? LPAREN type_name RPAREN
    | NOT? SUBSTITUTABLE AT ALL LEVELS
    ;
...
constraint_clauses
    : ADD LPAREN (out_of_line_constraint (COMMA out_of_line_constraint)* | out_of_line_ref_constraint) RPAREN
...
check_constraint
    : CHECK LPAREN condition RPAREN DISABLE?
    ;
...
c_agent_in_clause
    : AGENT IN LPAREN expressions_ RPAREN
    ;

c_parameters_clause
    : PARAMETERS LPAREN c_external_parameter (COMMA c_external_parameter)* RPAREN
    ;
...
subtype_declaration
    : SUBTYPE identifier IS type_spec (RANGE expression DOTDOT expression)? (NOT NULL_)? SEMICOLON
    ;

// cursor_declaration incorportates curscursor_body and cursor_spec

cursor_declaration
    : CURSOR identifier (LPAREN parameter_spec (COMMA parameter_spec)* RPAREN)? (RETURN type_spec)? (
        IS select_statement
    )? SEMICOLON
    ;
...
pragma_declaration
    : PRAGMA (
        ...
        | EXCEPTION_INIT LPAREN exception_name COMMA numeric_negative RPAREN
        | INLINE LPAREN id1 = identifier COMMA expression RPAREN
        | RESTRICT_REFERENCES LPAREN (identifier | DEFAULT) (COMMA identifier)+ RPAREN
        | DEPRECATE LPAREN identifier ( COMMA CHAR_STRING)? RPAREN
    ) SEMICOLON
    ;
...
record_type_def
    : RECORD LPAREN field_spec (COMMA field_spec)* RPAREN
    ;
...
varray_type_def
    : (VARRAY | VARYING? ARRAY) LPAREN expression RPAREN OF type_spec (NOT NULL_)?
    ;
...
cursor_loop_param
    : index_name IN REVERSE? lower_bound range_separator = DOTDOT upper_bound
    | record_name IN (cursor_name (LPAREN expressions_? RPAREN)? | LPAREN select_statement RPAREN)
    ;
...
call_statement
    : CALL? routine_name function_argument? (DOT routine_name function_argument?)* (
        INTO bind_variable
    )?
    ;
...
open_statement
    : OPEN cursor_name (LPAREN expressions_? RPAREN)?
    ;
...
collection_method_call // collection methods invocation that could be used as a statement
    : expression DOT (
        (DELETE | EXTEND) (LPAREN index += expression (COMMA index += expression)* RPAREN)?
        | TRIM (LPAREN index += expression RPAREN)?
    )
    ;
...
explain_statement
    : EXPLAIN PLAN (SET STATEMENT_ID EQUAL quoted_string)? (INTO tableview_name)? FOR (
        select_statement
        | update_statement
        | delete_statement
        | insert_statement
...
subquery_factoring_clause
    : query_name paren_column_list? AS LPAREN subquery order_by_clause? RPAREN search_clause? cycle_clause?
    ;
...
subav_factoring_clause
    : subav_name = id_expression ANALYTIC VIEW AS LPAREN subav_clause RPAREN
    ;
...
hierarchies_clause
    : HIERARCHIES LPAREN hier_alias += object_name (COMMA hier_alias += object_name)* RPAREN
    ;

filter_clauses
    : FILTER FACT LPAREN filter_clause (COMMA filter_clause)* RPAREN
    ;
...
add_calcs_clause
    : ADD MEASURES LPAREN add_calc_meas_clause (COMMA add_calc_meas_clause)* RPAREN
    ;

add_calc_meas_clause
    : meas_name = id_expression AS LPAREN expression RPAREN
    ;
...
table_ref_aux_internal
    ...
    | LPAREN table_ref subquery_operation_part* RPAREN (pivot_clause | unpivot_clause)? # table_ref_aux_internal_two
    | ONLY LPAREN dml_table_expression_clause RPAREN                                    # table_ref_aux_internal_thre
    ;
...
query_partition_clause
    : PARTITION BY ((LPAREN (subquery | expressions_)? RPAREN) | expressions_)
    ;
...
pivot_clause
    : PIVOT XML? LPAREN pivot_element (COMMA pivot_element)* pivot_for_clause pivot_in_clause RPAREN table_alias?
    ;

pivot_element
    : (numeric_function | aggregate_function_name LPAREN expression RPAREN) column_alias?
    ;
...
pivot_in_clause
    : IN LPAREN (subquery | ANY (COMMA ANY)* | pivot_in_clause_element (COMMA pivot_in_clause_element)*) RPAREN
    ;
...
unpivot_clause
    : UNPIVOT ((INCLUDE | EXCLUDE) NULLS)? LPAREN (column_name | paren_column_list) pivot_for_clause unpivot_in_clause RPAREN table_alias?
    ;

unpivot_in_clause
    : IN LPAREN unpivot_in_elements (COMMA unpivot_in_elements)* RPAREN
    ;

unpivot_in_elements
    : (column_name | paren_column_list) (AS (constant | LPAREN constant (COMMA constant)* RPAREN))?
    ;
...
rollup_cube_clause
    : (ROLLUP | CUBE) LPAREN grouping_sets_elements (COMMA grouping_sets_elements)* RPAREN
    ;

grouping_sets_clause
    : GROUPING SETS LPAREN grouping_sets_elements (COMMA grouping_sets_elements)* RPAREN
    ;
...
reference_model
    : REFERENCE reference_model_name ON LPAREN subquery RPAREN model_column_clauses cell_reference_options*
    ;
...
model_column_list
    : LPAREN model_column (COMMA model_column)* RPAREN
    ;
...
model_rules_clause
    : model_rules_part? LPAREN (model_rules_element (COMMA model_rules_element)*)? RPAREN
    ;
...
model_rules_element
    : (UPDATE | UPSERT ALL?)? cell_assignment order_by_clause? EQUAL expression
    ;
...
model_iterate_clause
    : ITERATE LPAREN expression RPAREN until_part?
    ;

until_part
    : UNTIL LPAREN condition RPAREN
    ;
...
update_set_clause
    : SET (
        ...
        | VALUE LPAREN identifier RPAREN EQUAL expression
    )
    ;

column_based_update_set_clause
    : column_name EQUAL expression
    | paren_column_list EQUAL subquery
    ;
...
values_clause
    : VALUES (REGULAR_ID | LPAREN expressions_ RPAREN | collection_expression)
    ;

merge_statement
    : MERGE INTO tableview_name table_alias? USING selected_tableview ON LPAREN condition RPAREN (
...
selected_tableview
    : (tableview_name | LPAREN select_statement RPAREN) table_alias?
    ;
...
general_table_ref
    : (dml_table_expression_clause | ONLY LPAREN dml_table_expression_clause RPAREN) table_alias?
    ;
...
dml_table_expression_clause
    ...
    | LPAREN select_statement subquery_restriction_clause? RPAREN
    | tableview_name hierarchies_clause? sample_clause?
    | json_table_clause (AS identifier)?
    | LATERAL LPAREN subquery subquery_restriction_clause? RPAREN
    ;

table_collection_expression
    : (TABLE | THE) (LPAREN subquery RPAREN | LPAREN expression RPAREN outer_join_sign?)
    ;
...
sample_clause
    : SAMPLE BLOCK? LPAREN expression (COMMA expression)? RPAREN seed_part?
    ;

seed_part
    : SEED LPAREN expression RPAREN
    ;
...
logical_operation
    : (
        ...
        | A_LETTER SET
        | EMPTY_
        | OF TYPE? LPAREN ONLY? type_spec (COMMA type_spec)* RPAREN
...
relational_operator
    : EQUAL
    | (NOT_EQUAL_OP | LT GT | EXCLAMATION EQUAL | CARET EQUAL)
    | (LT | GT) EQUAL?
    ;

in_elements
    : LPAREN subquery RPAREN
    | LPAREN concatenation (COMMA concatenation)* RPAREN
    | constant
    | bind_variable
    | general_element
    ;
...
interval_expression
    : DAY (LPAREN concatenation RPAREN)? TO SECOND (LPAREN concatenation RPAREN)?
    | YEAR (LPAREN concatenation RPAREN)? TO MONTH
    | concatenation (SECOND | DAY | MONTH | YEAR)
    ;

model_expression
    : unary_expression (LBRACKET model_expression_element RBRACKET)?
    ;
...
multi_column_for_loop
    : FOR paren_column_list IN LPAREN (subquery | LPAREN expressions_? RPAREN) RPAREN
    ;
...
unary_expression
    ...
    | unary_expression DOT (
        ...
        | (EXISTS | NEXT | PRIOR) LPAREN index += expression RPAREN
    )
...
implicit_cursor_expression
    : SQL (
        PERCENT_BULK_ROWCOUNT LPAREN index = expression RPAREN
        | PERCENT_BULK_EXCEPTIONS (DOT COUNT | LPAREN expression RPAREN DOT (ERROR_INDEX | ERROR_CODE))
    )
    ;

collection_expression
    : collation_name LPAREN expression RPAREN (DOT general_element_part)*
    ;
...
atom
    ...
    | LPAREN subquery RPAREN subquery_operation_part*
    | LPAREN expressions_ RPAREN
    ;

quantified_expression
    : (SOME | EXISTS | ALL | ANY) (
        LPAREN select_only_statement RPAREN
        | LPAREN expression (COMMA expression)* RPAREN
    )
    ;
...
string_function
    ...
    | TO_CHAR LPAREN (table_element | standard_function | expression) (COMMA quoted_string)? (
        COMMA quoted_string
    )? RPAREN
    ...
    | TRIM LPAREN ((LEADING | TRAILING | BOTH)? expression? FROM)? concatenation RPAREN
    | TO_DATE LPAREN (table_element | standard_function | expression) (
        DEFAULT concatenation ON CONVERSION ERROR
    )? (COMMA quoted_string (COMMA quoted_string)?)? RPAREN
    ;
...
json_function
    : JSON_ARRAY LPAREN json_array_element (COMMA json_array_element)* json_on_null_clause? json_return_clause? STRICT? RPAREN
    | JSON_ARRAYAGG LPAREN expression (FORMAT JSON)? order_by_clause? json_on_null_clause? json_return_clause? STRICT? RPAREN
    | JSON_OBJECT LPAREN json_object_content RPAREN
    | JSON_OBJECTAGG LPAREN KEY? expression VALUE expression ((NULL_ | ABSENT) ON NULL_)? (
        RETURNING (VARCHAR2 (LPAREN UNSIGNED_INTEGER ( BYTE | CHAR)? RPAREN)? | CLOB | BLOB)
    )? STRICT? (WITH UNIQUE KEYS)? RPAREN
    | JSON_QUERY LPAREN expression (FORMAT JSON)? COMMA CHAR_STRING json_query_returning_clause json_query_wrapper_clause? json_query_on_error_clause?
        json_query_on_empty_clause? RPAREN
    | JSON_SERIALIZE LPAREN CHAR_STRING (RETURNING json_query_return_type)? PRETTY? ASCII? TRUNCATE? (
        (NULL_ | ERROR | EMPTY_ (ARRAY | OBJECT)) ON ERROR
    )? RPAREN
    | JSON_TRANSFORM LPAREN expression COMMA json_transform_op (COMMA json_transform_op)* RPAREN
    | JSON_VALUE LPAREN expression (FORMAT JSON)? (
        COMMA CHAR_STRING? json_value_return_clause? ((ERROR | NULL_ | DEFAULT literal)? ON ERROR)? (
            (ERROR | NULL_ | DEFAULT literal)? ON EMPTY_
        )? json_value_on_mismatch_clause? RPAREN
    )?
    ;
...
json_table_clause
    : JSON_TABLE LPAREN expression (FORMAT JSON)? (COMMA CHAR_STRING)? ((ERROR | NULL_) ON ERROR)? (
        (EMPTY_ | NULL_) ON EMPTY_
    )? json_column_clause? RPAREN
    ;
...
json_transform_op
    ...
    | INSERT CHAR_STRING EQUAL CHAR_STRING ((REPLACE | IGNORE | ERROR) ON EXISTING)? (
        (NULL_ | IGNORE | ERROR | REMOVE)? ON NULL_
    )?
    | REPLACE CHAR_STRING EQUAL CHAR_STRING ((CREATE | IGNORE | ERROR) ON MISSING)? (
        (NULL_ | IGNORE | ERROR)? ON NULL_
    )?
    | expression (FORMAT JSON)?
    | APPEND CHAR_STRING EQUAL CHAR_STRING ((CREATE | IGNORE | ERROR) ON MISSING)? (
        (NULL_ | IGNORE | ERROR)? ON NULL_
    )?
    | SET CHAR_STRING EQUAL expression (FORMAT JSON)? ((REPLACE | IGNORE | ERROR) ON EXISTING)? (
        (CREATE | IGNORE | ERROR) ON MISSING
    )? ((NULL_ | IGNORE | ERROR)? ON NULL_)?
    ;

json_column_clause
    : COLUMNS LPAREN json_column_definition (COMMA json_column_definition)* RPAREN
    ;
...
json_column_definition
    ...
    | NESTED PATH? expression (LBRACKET ASTERISK RBRACKET)? json_column_clause
...
json_query_return_type
    : VARCHAR2 (LPAREN UNSIGNED_INTEGER ( BYTE | CHAR)? RPAREN)?
    | CLOB
    | BLOB
    ;
...
json_value_return_type
    : VARCHAR2 (LPAREN UNSIGNED_INTEGER ( BYTE | CHAR)? RPAREN)? TRUNCATE?
    | CLOB
    | DATE
    | NUMBER (LPAREN INTEGER (COMMA INTEGER)? RPAREN)?
...
json_value_on_mismatch_clause
    : (IGNORE | ERROR | NULL_) ON MISMATCH (LPAREN MISSING DATA | EXTRA DATA | TYPE ERROR RPAREN)?
    ;
...
numeric_function
    : SUM LPAREN (DISTINCT | ALL)? expression RPAREN
    | COUNT LPAREN (ASTERISK | ((DISTINCT | UNIQUE | ALL)? concatenation)?) RPAREN over_clause?
    | ROUND LPAREN expression (COMMA UNSIGNED_INTEGER)? RPAREN
    | AVG LPAREN (DISTINCT | ALL)? expression RPAREN
    | MAX LPAREN (DISTINCT | ALL)? expression RPAREN
...
other_function
    ...
    | COUNT LPAREN (ASTERISK | (DISTINCT | UNIQUE | ALL)? concatenation) RPAREN over_clause?
    | (CAST | XMLCAST) LPAREN (MULTISET LPAREN subquery RPAREN | concatenation) AS type_spec (
        DEFAULT concatenation ON CONVERSION ERROR
    )? (COMMA quoted_string (COMMA quoted_string)?)? RPAREN
    | COALESCE LPAREN table_element (COMMA (numeric | quoted_string))? RPAREN
    | COLLECT LPAREN (DISTINCT | UNIQUE)? concatenation collect_order_by_part? RPAREN
    | within_or_over_clause_keyword function_argument within_or_over_part+
    | LISTAGG LPAREN (ALL | DISTINCT | UNIQUE)? argument (COMMA string_delimiter)? listagg_overflow_clause? RPAREN (
        WITHIN GROUP LPAREN order_by_clause RPAREN
    )? over_clause?
    | cursor_name (PERCENT_ISOPEN | PERCENT_FOUND | PERCENT_NOTFOUND | PERCENT_ROWCOUNT)
    | DECOMPOSE LPAREN concatenation (CANONICAL | COMPATIBILITY)? RPAREN
    ...
    | standard_prediction_function_keyword LPAREN expressions_ cost_matrix_clause? using_clause? RPAREN
    | (TO_BINARY_DOUBLE | TO_BINARY_FLOAT | TO_NUMBER | TO_TIMESTAMP | TO_TIMESTAMP_TZ) LPAREN concatenation (
        DEFAULT concatenation ON CONVERSION ERROR
    )? (COMMA quoted_string (COMMA quoted_string)?)? RPAREN
    | (TO_DSINTERVAL | TO_YMINTERVAL) LPAREN concatenation (DEFAULT concatenation ON CONVERSION ERROR)? RPAREN
    | TRANSLATE LPAREN expression (USING (CHAR_CS | NCHAR_CS))? (COMMA expression)* RPAREN
    | TREAT LPAREN expression AS REF? type_spec RPAREN (DOT general_element_part)*
    | TRIM LPAREN ((LEADING | TRAILING | BOTH)? quoted_string? FROM)? concatenation RPAREN
    | VALIDATE_CONVERSION LPAREN concatenation AS type_spec (COMMA quoted_string (COMMA quoted_string)?)? RPAREN
    | XMLAGG LPAREN expression order_by_clause? RPAREN (DOT general_element_part)*
    | (XMLCOLATTVAL | XMLFOREST) LPAREN xml_multiuse_expression_element (
        COMMA xml_multiuse_expression_element
    )* RPAREN (DOT general_element_part)*
    | XMLELEMENT LPAREN (ENTITYESCAPING | NOENTITYESCAPING)? (NAME | EVALNAME)? expression (
        /*TODO{input.LT(2).getText().equalsIgnoreCase("xmlattributes")}?*/ COMMA xml_attributes_clause
    )? (COMMA expression column_alias?)* RPAREN (DOT general_element_part)*
    | XMLEXISTS LPAREN expression xml_passing_clause? RPAREN
    | XMLPARSE LPAREN (DOCUMENT | CONTENT) concatenation WELLFORMED? RPAREN (DOT general_element_part)*
    | XMLPI LPAREN (NAME identifier | EVALNAME concatenation) (COMMA concatenation)? RPAREN (
        DOT general_element_part
    )*
    | XMLQUERY LPAREN concatenation xml_passing_clause? RETURNING CONTENT (NULL_ ON EMPTY_)? RPAREN (
        DOT general_element_part
    )*
    | XMLROOT LPAREN concatenation (COMMA xmlroot_param_version_part)? (
        COMMA xmlroot_param_standalone_part
    )? RPAREN (DOT general_element_part)*
    | XMLSERIALIZE LPAREN (DOCUMENT | CONTENT) concatenation (AS type_spec)? xmlserialize_param_enconding_part? xmlserialize_param_version_part?
        xmlserialize_param_ident_part? ((HIDE | SHOW) DEFAULTS)? RPAREN (DOT general_element_part)?
    | TIME CHAR_STRING
    | xmltable
    ;
...
cost_matrix_clause
    : COST (
        MODEL AUTO?
        | LPAREN cost_class_name (COMMA cost_class_name)* RPAREN VALUES LPAREN expressions_? RPAREN
    )
    ;
...
xml_attributes_clause
    : XMLATTRIBUTES LPAREN (ENTITYESCAPING | NOENTITYESCAPING)? (SCHEMACHECK | NOSCHEMACHECK)? xml_multiuse_expression_element (
        COMMA xml_multiuse_expression_element
    )* RPAREN
    ;

xml_namespaces_clause
    : XMLNAMESPACES LPAREN (concatenation column_alias)? (COMMA concatenation column_alias)* xml_general_default_part? RPAREN
    ;
...
xmlserialize_param_ident_part
    : NO INDENT
    | INDENT (SIZE EQUAL concatenation)?
    ;
...
partition_extension_clause
    : (SUBPARTITION | PARTITION) FOR? LPAREN expressions_? RPAREN
    ;
...
routine_name
    : identifier (DOT id_expression)* (AT link_name)?
    ;
...
variable_name
    : (INTRODUCER char_set_name)? id_expression (DOT id_expression)?
    | bind_variable
    ;
...
link_name
    : database (DOT domain)* (AT_SIGN connection_qualifier)?
    ;
...
xmltable
    : XMLTABLE LPAREN (xml_namespaces_clause COMMA)? concatenation xml_passing_clause? (
...
keep_clause
    : KEEP LPAREN DENSE_RANK (FIRST | LAST) (query_partition_clause | order_by_clause) RPAREN over_clause?
    ;

function_argument
    : LPAREN (argument (COMMA argument)*)? RPAREN keep_clause?
    ;

function_argument_analytic
    : LPAREN (argument respect_or_ignore_nulls? (COMMA argument respect_or_ignore_nulls?)*)? RPAREN keep_clause?
    ;

function_argument_modeling
    : LPAREN column_name (COMMA (numeric | NULL_) (COMMA (numeric | NULL_))?)? USING (
        tableview_name DOT ASTERISK
        | ASTERISK
...
argument
    : (identifier EQUAL GT)? expression
    ;
...
datatype
    ...
    | INTERVAL (YEAR | DAY) (LPAREN expression RPAREN)? TO (MONTH | SECOND) (LPAREN expression RPAREN)?
    ;

precision_part
    : LPAREN (numeric | ASTERISK) (COMMA (numeric | numeric_negative))? (CHAR | BYTE)? RPAREN
    ;
...
bind_variable
    ...
    (INDICATOR? (BINDVAR | COLON UNSIGNED_INTEGER))? (DOT general_element_part)*
    ;

general_element
    : general_element_part
    | general_element (DOT general_element_part)+
    | LPAREN general_element RPAREN
    ;

general_element_part
    : (INTRODUCER char_set_name)? id_expression (AT link_name)? function_argument*
    ;
...
constant
    ...
    | INTERVAL (quoted_string | bind_variable | general_element_part) (
        ...
        | MINUTE
        | SECOND
    ) (LPAREN (UNSIGNED_INTEGER | bind_variable) (COMMA (UNSIGNED_INTEGER | bind_variable))? RPAREN)? (
        TO (MONTH | DAY | HOUR | MINUTE | SECOND (LPAREN (UNSIGNED_INTEGER | bind_variable) RPAREN)?)
    )?
    | numeric
    | DATE quoted_string
    | quoted_string
